...
public class Parser {
	...
	private static final String C_PUSH_REGEX = WHITESPACE + "push" + WHITESPACE
			+ SEGMENT + WHITESPACE + "([^\\d](\\w|\\.|\\:)+\\.)?"+"(\\d+)" + WHITESPACE +
			COMMENT_REGEX + "?";
	...
	{
		if (!inputFile.getName().equals("AllVMFiles.vm"))
		...
		while (hasMoreCommands()) {
			...
			if (matcher.matches()) {
				...
				if (segment.matches("static")){
					writer.println("push " + segment + " " + getFileName() + matcher.group(4));
					advance();
					continue;
				}
	...
	public VMCommand getCommandType() throws IllegalArgumentException
	{
		...
		if (matcher.matches())
		{
			...
			return new C_Push(segment, value, newName);
		}
		pattern = Pattern.compile(C_POP_REGEX);
		matcher = pattern.matcher(currentLine);
		if (matcher.matches())
		{
			...
			if (value >= MAX_MEMORY_VALUE+1)
				throw new IllegalArgumentException();
			return new C_Pop(segment, value, newName);
		}
		pattern = Pattern.compile(C_FUNCTION_REGEX);
		matcher = pattern.matcher(currentLine);
		if (matcher.matches())
